...
;; marker regions which are edited in a separate `noweb-mode2'.  (It's
...
Supports differnt major modes for doc and code chunks using multi-mode."
  ...
  (flet ((noweb-mode2 ()))
    ...
    (let ((multi-mode-alist (list (cons 'noweb-mode2 #'noweb-select-mode)
  ...
  (let ((noweb-doc-buffer (cdr (assq noweb-doc-mode
	...
	(noweb-code-buffer (cdr (assq noweb-code-mode
				      multi-indirect-buffers-alist)))
	(noweb-buffer (cdr (assq 'noweb-mode2
				 multi-indirect-buffers-alist))))
...
(defun noweb-select-mode (pos)
  ...
  (save-excursion
	...
	(cond
	 ((match-beginning 2)
	  (multi-make-list 'noweb-mode2 (match-beginning 0) (match-end 0)))
	 ((match-beginning 1)
	  (let (start)
	    ...
	    (while (looking-at "@ +%def")
	      (forward-line))
	    (multi-make-list 'noweb-mode2 start (line-end-position 0))))
	 (t 
	  ;; Else start of a doc chunk.  Mode is doc unless we're at bol.
	  (if (< pos (match-end 0))
	      (multi-make-list 'noweb-mode2 (match-beginning 0)
			       (1- (match-end 0)))
